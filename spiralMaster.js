...
var spiralMaster = function(options) {
    ...
    if (r_colorWheel === 'singleColor'){
        var hexColor = rgbToHex(options.startingRed, options.startingGreen, options.startingBlue);
    ...
    var colorRaphObjects = function(objectCollection){
        if (r_colorWheel === 'singleColor'){
            applySingleColorToRaphObj(objectCollection);
            return;
        }
    ...
    var applySingleColorToRaphObj = function(objectCollection){
        var collectionSize = objectCollection.length;
        for(var i = 0; i < collectionSize; i++){
            var item = objectCollection[i];
            item.attr('fill', r_getColor());
        }
    }
